// app/server.js
// MiddleGround server: auth (signup/login/logout/me) + meetings flow + venue suggestions.
...
// UPDATED: login by email OR username (case-insensitive)
...
const VENUE_QUERIES = {
  ...
  brunch:"brunch", dinner:"restaurant", bar:"bar",
  park:"park", museum:"museum", library:"library", mall:"shopping mall", gym:"gym",
  default:"restaurant",
};
...
async function searchFoursquare({lat,lng}, venueType, radiusMeters){
  ...
  if (!resp.ok) { console.error("Foursquare", resp.status, await resp.text()); return []; }
...
app.post("/api/meetings", async (req, res) => {
  try {
    ...
    const emails = (invitees||"").split(/[,\s;]+/).map(e=>e.trim()).filter(e=>e && isEmail(e));
    ...
    for (let i=0;i<all.length;i++){
      ...
      const ins = await query(
        `INSERT INTO invitations (meeting_id, email, invited_user_id, token, role, status)
         ...
         ON CONFLICT (meeting_id,email) DO UPDATE SET token=EXCLUDED.token, role=EXCLUDED.role
...
// Suggestions (need at least 2 locations)
...
app.post("/api/meetings/:id/finalize", async (req, res) => {
  try {
    ...
    const updated = await query(
      `UPDATE meetings SET status='finalized', finalized_place_json=$2
       WHERE id=$1
    ...
    await Promise.all(parts.rows.map(r =>
      sendMail({
        to: r.email,
...
app.listen(port, hostname, () => {
  ...
  if (!isProd && openOnStart) {
    ...
    try { child_process.exec(`${cmd} "${localUrl}/index.html"`); } catch {}
  }
});
